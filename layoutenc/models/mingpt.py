...
clip = np.load('./data/pretrained/vg_clip_embeds.npy')
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size, block_size, n_layer=12, n_head=8, n_embd=256,
        ...
        self.label_emb = nn.Embedding(class_num, 768)
        # project CLIP into layout embedding
        self.label = nn.Sequential(nn.Linear(768, config.n_embd),
                                   nn.ReLU(),
    ...
    def forward(self, idx, label, bbox, mask=None, embeddings=None, targets=None):
        ...
        # label.long(): bs, 30
        # label_emb: bs, 30, 768
        bs, num_objs = label.shape
        label_emb = []
        for i in range(bs):
            label_emb_bs = []
            for j in range(num_objs):
                label_index = label.long()[i, j].item()
                if label_index == 0:
                    label_emb_bs.append(self.label_emb(torch.tensor([0]).to(label.device).squeeze(0)))
                else:
                    label_emb_bs.append(clip[label_index].to(label.device)) # num_objs, num_embeds_clip
            label_emb.append(torch.stack(label_emb_bs))
        
        label_emb = torch.stack(label_emb).to(label.device) # bs, num_objs, num_embeds_clip
        label_emb = self.label(label_emb.float())
