...
class Net2NetTransformer(pl.LightningModule):
    def __init__(self, transformer_config, first_stage_config,
        ...
        if permuter_config is None:
            permuter_config = {"target": "layoutenc.modules.taming.modules.permuter.Identity"}
    ...
    def forward(self, x, label, bbox, mask, mode='train'):
        if mode == 'train':
            ...
            if self.training and self.pkeep < 1.0:
                image_token_mask = torch.bernoulli(self.pkeep*torch.ones(z_indices.shape,
                                                             device=z_indices.device))
                image_token_mask = image_token_mask.round().to(dtype=torch.int64)
                r_indices = torch.randint_like(z_indices, self.transformer.config.vocab_size)
                a_indices = image_token_mask*z_indices+(1-image_token_mask)*r_indices
            else:
                a_indices = z_indices
